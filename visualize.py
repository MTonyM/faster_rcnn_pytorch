...
def renderImgTable(inputImgs, ncoln, imgDir):
    ...
    for row in range(int(math.ceil(len(inputImgs) / ncoln))):
...
def writeImgHTML(inputImgs, iters, split, ncoln, output_dir):
    htmlFile = 'index.html'

    rootDir = os.path.join(output_dir, str(iters) + '_' + split)
